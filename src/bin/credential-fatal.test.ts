...
test('control: the binary under test exists and this harness can actually run it', async () => {
  ...
  // by way of a process that never started and a `code` that happened to match. The config dir is
  // an empty fixture: an empty variable counts as unset, and the developer's own config file would
  // fill it with a real controller and start a real host.
  const run = await runBinary({
    ...process.env,
    PERISCOPE_CONFIG_DIR: mkdtempSync(join(tmpdir(), 'periscope-control-')),
