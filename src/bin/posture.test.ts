...
test('a token posture with nothing cached says so on both lines, before any dial can refuse it', () => {
  const credential = { credential: {} as never, pairedHostId: null };
  const posture = describePosture(inputs({ credential, tokenExpiresAtMs: null }));
  assert.match(postureLine(posture), /credential token \(no cache yet - sign in first\)/);
  assert.match(
    renderPosture(posture).join('\n'),
    /credential: signed-in token \(no cache yet - run periscope login\)/,
  );
});

test('PERISCOPE_CONFIG_DIR is listed only when the environment set it: it is never a config-file key', () => {
  const set = renderPosture(describePosture(inputs())).join('\n');
  assert.match(set, /^ {2}PERISCOPE_CONFIG_DIR = \/secrets\/periscope {2}\[environment\]$/m);
  const unset = renderPosture(
    describePosture(
      inputs({
        raw: { PERISCOPE_CONTROLLER_URL: 'wss://c.example/link' },
        merged: { PERISCOPE_CONTROLLER_URL: 'wss://c.example/link' },
      }),
    ),
  ).join('\n');
  assert.doesNotMatch(unset, /PERISCOPE_CONFIG_DIR =/);
  assert.match(unset, /PERISCOPE_HOST_ID = \(unset\)/, 'the other unset keys still list');
});
