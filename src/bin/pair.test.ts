...
test("a controller whose certificate Node refuses is named as such, with the remedy, never as 'unreachable'", async () => {
  const store = new MemoryStore();
  const outcome = await runPair('c', ENV, {
    transport: async () => {
  ...
  assert.equal(outcome.ok, false);
  if (!outcome.ok) {
    assert.match(outcome.detail, /DEPTH_ZERO_SELF_SIGNED_CERT/);
    assert.match(outcome.detail, /NODE_EXTRA_CA_CERTS/);
    assert.doesNotMatch(outcome.detail, /could not be reached/);
  }
  assert.equal(store.written, null);
});
