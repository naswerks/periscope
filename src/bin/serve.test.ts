...
test('the paired posture names the host id the credential is bound to, and it wins over the configured one', () =>
  inDir(async (dir) => {
    ...
    // regression: the posture line leads; the credential lines that explain it follow.
    const hostAt = run.out.findIndex((line) => line.includes('[host]'));
    const credentialAt = run.out.findIndex((line) => line.includes('[credential]'));
    assert.ok(
      hostAt >= 0 && hostAt < credentialAt,
      `[host] must print before [credential]:\n${run.out.join('\n')}`,
    );
  }));
