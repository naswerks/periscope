...
test('regression: the emitted bin has an LF shebang', () => {
  ...
  // regression: npm's publish normalizer rejects a bin path spelled with a leading `./` and
  // STRIPS the entry, so the registry copy would ship no command at all; the pack and the
  // consumer smoke never see it because the tarball's manifest is not normalized the same way.
  assert.doesNotMatch(binPath, /^\.\//, 'the bin path must not start with ./ (npm publish removes it)');
  const emitted = readFileSync(at(binPath), 'utf8');
...
test('the package is public: `private` is absent and the publish gate is armed', () => {
  assert.equal(
    manifest.private,
    undefined,
    'a published package cannot carry `private`; the publish gate (scripts/publish-gate.mjs) is what guards a release',
  );
  assert.equal(
    manifest.publishConfig?.access,
    'public',
    'a scoped package publishes private unless it says otherwise',
  );
});
