...
const configurationProse = prose(configuration);
...
test('regression: the configuration document states the one-controller, one-replica constraint', () => {
  assert.match(
    configuration,
    /^## Two constraints, before you deploy/m,
    'the constraints section is missing',
  );
  assert.match(configurationProse, /One controller, one replica/, 'the replica constraint is not stated');
  assert.match(configurationProse, /gaps/, 'what a second replica would see is not stated');
});

test('regression: the configuration document states the protocol-version handshake constraint', () => {
  assert.match(
    configurationProse,
  ...
  assert.match(
    configurationProse,
    /retries with backoff, forever/i,
    'the consequence of a mismatch is not stated',
  );
});
...
test('the configuration document lists every PERISCOPE_ variable the shipped code reads', () => {
  ...
  assert.ok(environment.length > 500, 'docs/configuration.md has no Environment section');
  const undocumented = [...read_].filter((name) => !environment.includes(`\`${name}\``)).sort();
  assert.deepEqual(
    undocumented,
    [],
    `variables the code reads but docs/configuration.md does not list: ${undocumented.join(', ')}`,
  );
...
test('the README opens with an install line that names this package', () => {
  ...
  assert.ok(
    head.some((line) => line.startsWith(`npm install ${manifest.name}`)),
    `the first 40 lines carry no "npm install ${manifest.name}" line`,
  );
});

test("the examples document's minimal controller is the example file, byte for byte", () => {
  ...
  const section = examples.split('## `minimal-controller/`')[1] ?? '';
  const block = /```ts\n([\s\S]*?)```/.exec(section)?.[1];
  assert.ok(block !== undefined, 'examples/README.md has no ts block under the minimal controller');
  assert.equal(
    block,
    file,
    'the examples/README.md block and examples/minimal-controller/controller.ts differ; copy the file',
  );
});

test('the README and the configuration document walk a reader from install to a running host', () => {
  assert.match(readme, /^## Run a host/m, 'the walkthrough section is missing');
  assert.match(readmeProse, /periscope pair <code>/, 'the pairing command is not shown');
  assert.match(readmeProse, /claude auth login/, "the agent's own sign-in step is not shown");
  assert.match(
    configurationProse,
    /\[link\] idle -> connecting \(start_requested\)/,
    'the first link line is not shown',
  );
  assert.match(configurationProse, /refuses to start/i, 'the start-up refusals are not listed');
});
